...
``` 
首先我们需要知道，在Bootstrap中
$（window）.height（）表示当前可视区域的高度，随着浏览器的高度变化
$（document）.height（）表示整个页面的高度
$（window）.scrollTop（）表示当前滚动地方的窗口顶端到整个页面顶端的距离
所以：
当 scollTop > document height - window height 时即滑到了底部
```
